...
![과제3 실행 화면](img/screen4.png)

- 과제 내용
	- 사용자가 입력한 비밀번호를 직접 확인하거나 다시 숨길 수 있도록 체크박스 컨트롤을 추가하여 시각적 편의성을 높였습니다.
	- 보안을 위해 기본적으로는 마스킹 처리하되, 필요한 경우에만 선택적으로 평문으로 표시하는 기능을 구현하였습니다.

- 구현 내용과 기능 설명
	- 체크박스의 CheckedChanged 이벤트를 활용하여, 체크 상태일 때는 비밀번호 마스킹을 해제하고 체크 해제 시 다시 마스킹되도록 로직을 구성하였습니다.
	- 텍스트 박스의 PasswordChar 속성을 동적으로 변경하여, 체크 시에는 문자 그대로 보여주고 해제 시에는 다시 특정 암호 문자로 가려지도록 구현하였습니다.
	- 사용자가 체크박스를 클릭하는 즉시 입력창의 텍스트 형식이 변하게 하여 별도의 확인 절차 없이도 현재 설정 상태를 바로 알 수 있게 설계하였습니다.
	- 오타 입력을 방지하기 위해 비밀번호를 확인하고 싶어 하는 사용자의 요구사항을 반영하여 로그인 성공률을 높이는 UX 개선 작업을 진행하였습니다.